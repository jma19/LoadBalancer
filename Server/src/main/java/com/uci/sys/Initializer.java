...
import com.uci.conf.ApplicationRepository;
...
public class Initializer {
    ...
    public void init() {
        try {
            ...
            Integer port = applicationRepository.getSelfPort();
            List<NameValuePair> params = new ArrayList<>();
            params.add(new BasicNameValuePair("ip", ip));
            params.add(new BasicNameValuePair("port", String.valueOf(port)));
            String response = HttpUtils.post("http://localhost:8000/loadbalancer/register", params);
